...
class AsyncCacheRefresher(metaclass=Singleton):
    ...
    def worker(worker_id: int, worker_queue: queue.Queue):
        ...
        while True:
            ...
            try:
                ...
                with scopes_disabled():
                    try:
                        item.task.mark_failed(str(e))
                    except BaseException:
                        logger.exception(f"Failed to mark task {item.task.id} as failed")
            finally:
    ...
    def process_refresh_work(work: RefreshWork, logger: logging.Logger):
        ...
        try:
            ...
            all_cache_keys = []
            for pattern in work.cache_patterns:
            ...
            logger.debug(f"Found {task.total_items} cache keys to refresh for task {task.id}")

            with scope(space=space):
                batch_size = task.batch_size
                for i in range(0, len(all_cache_keys), batch_size):
            ...
            try:
                ...
                logger.exception(f"Failed to mark task {work.task.id} as failed")
            raise
